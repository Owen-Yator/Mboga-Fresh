...
const VendorCartPage = () => {
  ...
  const handleDecreaseQty = (item) => {
    // This logic is flawed, but matches your original file
    if (item.qty > 1) {
      const updatedItem = { ...item, qty: item.qty - 1 };
      removeItem(item.id); // This will remove it, then re-add with new qty
      addItem(updatedItem, updatedItem.qty);
    } else {
      removeItem(item.id);
    }
  };
    ...
    return (
      <div className="max-w-4xl mx-auto text-center py-16">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-8">
          Browse the Farmily Marketplace to find great bulk products from local
          farmers.
        </p>
        <button
              ...
              <div className="divide-y divide-gray-200 dark:divide-gray-700">
                {items.map((item) => {
                  const priceValue = Number(
                    String(item.price).match(/(\d+(\.\d+)?)/)?.[0] || 0
                  );
